  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto">
      ...
      <div className="flex flex-col items-center pt-6 pb-4 px-4">
        {/* Single Arc Reactor Logo - No duplicate */}
        <div className="mb-3">
          <ArcReactorLogo size={100} />
        </div>

        {/* Title */}
        <h1 className="jarvis-text-shimmer text-2xl md:text-3xl font-black tracking-[0.2em] mb-1">
          J.A.R.V.I.S
        </h1>
        <p className="text-[9px] md:text-[10px] tracking-widest uppercase mb-1" style={{ color: '#90a8cc' }}>
          {language === 'ar' ? 'نظام ذكاء اصطناعي متقدم' : 'Advanced AI System'}
        </p>

        {/* Time & Greeting */}
        <div className="flex flex-col items-center mt-3 mb-4">
          <div className="text-2xl md:text-4xl font-mono tracking-[0.15em] mb-1" style={{ color: '#d0e4f8' }}>
            {time}
          </div>
          <p className="text-[12px] mb-1" style={{ color: '#00e5ff' }}>{greeting}</p>
        ...
        <div className="flex items-center gap-3 mb-4">
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full" style={{ background: 'rgba(16, 185, 129, 0.1)', border: '1px solid rgba(16, 185, 129, 0.2)' }}>
            ...
            <span className="text-[9px] tracking-wider uppercase" style={{ color: '#10b981' }}>{translations.online}</span>
          </div>
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full" style={{ background: 'rgba(0, 229, 255, 0.1)', border: '1px solid rgba(0, 229, 255, 0.2)' }}>
            <span className="text-[9px]" style={{ color: '#90a8cc' }}>Session:</span>
            <span className="text-[9px] font-mono" style={{ color: '#00e5ff' }}>{formatUptime(uptime)}</span>
          </div>
        </div>
      </div>
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between mb-1.5">
                  ...
                  <div className="w-1 h-1 rounded-full" style={{ background: module.color }} />
                </div>
                <h3 className="text-[10px] font-bold mb-0.5" style={{ color: '#d0e4f8' }}>{module.label}</h3>
                <p className="text-[8px]" style={{ color: 'rgba(144, 168, 204, 0.5)' }}>{module.desc}</p>
              </button>
            );
          })}
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between mb-1.5">
                  ...
                  <div className="px-1 py-0.5 rounded text-[6px] uppercase" style={{ background: `${tool.color}20`, color: tool.color }}>
                    New
                  </div>
                </div>
                <h3 className="text-[10px] font-bold mb-0.5" style={{ color: '#d0e4f8' }}>{tool.label}</h3>
                <p className="text-[8px]" style={{ color: 'rgba(144, 168, 204, 0.5)' }}>{tool.desc}</p>
              </button>
            );
          })}
        </div>
      ...
      <div className="px-4 pb-20 md:pb-4">
        <div className="jarvis-hud-card p-3">
          ...
          <div className="grid grid-cols-4 gap-2 text-center">
            <div>
              <div className="text-base font-bold" style={{ color: '#00e5ff' }}>24</div>
              <div className="text-[7px] uppercase" style={{ color: 'rgba(144, 168, 204, 0.4)' }}>
                {language === 'ar' ? 'رسائل' : 'Messages'}
              </div>
            </div>
            <div>
              <div className="text-base font-bold" style={{ color: '#7c5cff' }}>128</div>
              <div className="text-[7px] uppercase" style={{ color: 'rgba(144, 168, 204, 0.4)' }}>
                {language === 'ar' ? 'صور' : 'Images'}
              </div>
            </div>
            <div>
              <div className="text-base font-bold" style={{ color: '#f59e0b' }}>12</div>
              <div className="text-[7px] uppercase" style={{ color: 'rgba(144, 168, 204, 0.4)' }}>
                {language === 'ar' ? 'فيديو' : 'Videos'}
              </div>
            </div>
            <div>
              <div className="text-base font-bold" style={{ color: '#10b981' }}>8</div>
              <div className="text-[7px] uppercase" style={{ color: 'rgba(144, 168, 204, 0.4)' }}>
